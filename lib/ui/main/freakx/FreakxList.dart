...
import 'FreakxController.dart';
import 'FreakxDetails.dart';
import 'FreakxWebview.dart';

class FreakxList extends StatefulWidget {
  var gameid1;
  String url1;
  String name1 = "";

  FreakxList(this.gameid1, this.url1, this.name1);

  GameJobListS createState() => GameJobListS(gameid1, url1, name1);
}

class GameJobListS extends State<FreakxList> with WidgetsBindingObserver {
  var gameid;
  String url;
  String name = "";

  GameJobListS(this.gameid, this.url, this.name);

  FreakxController controller;
  ...
  Widget build(BuildContext context) {
    controller = Get.put(FreakxController(gameid));
    ...
    return RefreshIndicator(
      ...
      onRefresh: () {
        return Future.delayed(const Duration(seconds: 1), () async {
          ...
          /*       try {
            if (controller.unity_history_userRegistrations
                !=null && controller.unity_history_userRegistrations
                .length >
                0) {
          ...
          // controller.getUnityHistory(gameid);
...
*/
          ...
          : controller.unityEventList.value.data[index].lootBoxEvent == true &&
              ...
              ? GestureDetector(
                  onTap: () async {
                    ...
                    /* WalletPageController walletPageController =
                        Get.put(WalletPageController());
                    walletPageController.alertLookBoxGameContest(controller
                        .unityEventList.value.data[index].id); //zero_amount*/
  ...
  Widget listHistoryUnity(BuildContext context, int index) {
    return InkWell(
      onTap: () {
        ...
        Get.to(() => FreakxDetails(url != null ? url : "",
  ...
  Future<Map> getPreJoinEventFreakx(
    ...
    if (response != null && response['statusCode'] == null) {
      ...
      if (preJoinResponseModel != null) {
        ...
        if (deficitAmount.value > 0) {
          ...
          Utils()
              .customPrint('contestAmount freakx ${AppString.contestAmount}');
          Utils().alertInsufficientBalance(context);
        } else {
          if (amount <= 0) {
            base_controller.showPreJoinBoxFree(
                "freakx",
              ...
              child: Column(
                children: [
                  ...
                  _Button(context, "CONFIRM_SAVE".tr,
                      preJoinResponseModel.webViewUrl, event_id, url),
                  SizedBox(
                    height: 0,
                  ),
                ],
  ...
  Widget _Button(BuildContext context, String values, String gameZopWebViewUrl,
                          ...
                          Obx(() => GestureDetector(
                                onTap: () {
                                  if (walletPageController
                                    ...
                                    getPreJoinEventFreakxGameContestLootbox(
                                        controller.unityEventList.value
                                            .data[index].id,
                                        context,
                                        controller
  ...
  Future<Map> getPreJoinEventFreakxGameContestLootbox(
      String event_id, BuildContext context, int amount) async {
    // Utils().customPrint("event_id ===>${contest_id}");
    Utils().customPrint("user_id ===> ${controller.user_id}");
    final param = {
    ...
    if (response != null && response['statusCode'] == null) {
      ...
      if (preJoinResponseModel != null) {
        ...
        if (deficitAmount.value > 0) {
          ...
          open_game(event_id, preJoinResponseModel.webViewUrl);
        }
      } else {
        AppBaseResponseModel appBaseErrorModel =
            AppBaseResponseModel.fromMap(response);
  ...
  void open_game(var event_id, var gameZopWebViewUrl) {
    Utils().customPrint("LOCATION 10: ---------SUCCESS");
